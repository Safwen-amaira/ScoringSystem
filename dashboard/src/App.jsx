...
function App() {
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel"><PanelHeader eyebrow="AI Assistant" title="Cybersecurity engineering assistant" actions={<span className="mini-badge">H-Brain 0.2.0 (Reasoning)</span>} /><div className="chat-thread" ref={chatThreadRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}>{chatMessages.map((message, index) => {
    ...
    return <article key={`${message.role}-${index}`} className={`chat-bubble ${message.role}`}><span>{isAssistant ? "H-Brain Expert" : "Operator"}</span>{thought ? <details className="chat-thought" open><summary>Strategic Reasoning</summary><p>{thought}</p></details> : null}<p>{answer}</p></article>;
  })}{busy === "chat" ? <article className="chat-bubble assistant"><span>H-Brain Specialist</span><p className="thinking-dots">Analyzing telemetry and coordinating response</p></article> : null}</div><form className="chat-form" onSubmit={handleChatSubmit}><textarea value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleChatSubmit(event); } }} placeholder="Direct access to H-Brain: Ask about triage, containment, or banking SOC playbooks..." /><button type="submit" style={{ background: "var(--gold-gradient)", color: "#000", fontWeight: "bold" }}>Initiate Consultation</button></form></section> : null}
