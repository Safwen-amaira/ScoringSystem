...
function severityColor(value) {
  return ({ critical: "#dc2626", high: "#f97316", medium: "#eab308", low: "#22c55e" }[(value || "low").toLowerCase()] || "#22c55e");
}
...
function App() {
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel premium-chat"><div className="chat-interface"><div className="chat-header"><div><h3>H-Brain Expert</h3><p>Production-Grade Security Reasoning</p></div><div className="chat-status"><span className="status-dot"></span>Active Reasoning</div></div><div className="chat-thread" ref={chatThreadRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}><div className="chat-content-container">{chatMessages.map((message, index) => {
    ...
    return <article key={`${message.role}-${index}`} className={`message-row ${message.role}`}><div className="message-avatar">{isAssistant ? <img src={LOGO_URL} alt="H-Brain" /> : <div className="user-icon-avatar"><Icon name="user" /></div>}</div><div className="message-body"><strong>{isAssistant ? "H-Brain" : "You"}</strong><div className="message-text">{thought ? <details className="chat-thought" open><summary>Strategic Reasoning</summary><p>{thought}</p></details> : null}<p>{answer}</p></div>{isAssistant ? <div className="message-actions"><button className="chat-action-btn" onClick={() => copyToClipboard(answer)} title="Copy message" type="button"><Icon name="copy" /></button><button className="chat-action-btn" onClick={() => { setChatDraft(chatMessages[index - 1]?.content || ""); handleChatSubmit({ preventDefault: () => {} }); }} title="Regenerate" type="button"><Icon name="refresh" /></button></div> : null}</div></article>;
  })}{busy === "chat" ? <article className="message-row assistant loading"><div className="message-avatar"><img src={LOGO_URL} alt="H-Brain" /></div><div className="message-body"><strong>H-Brain</strong><div className="shimmering-bar" /></div></article> : null}</div></div><form className="chat-input-area" onSubmit={handleChatSubmit}><div className="input-pill"><textarea value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleChatSubmit(event); } }} placeholder="Direct access to H-Brain: Ask about triage, containment, or banking SOC playbooks..." /></div><button type="submit" className="send-btn" disabled={busy === "chat" || !chatDraft.trim()}><Icon name="chat" /><span>Send</span></button></form></div></section> : null}
