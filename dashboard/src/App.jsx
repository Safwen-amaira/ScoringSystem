...
const LOGO_URL = "https://i.ibb.co/JWMJPnc6/hanicar.png";
...
function Icon({ name }) {
  const paths = {
    ...
    lock: <path d="M12 2a4 4 0 0 1 4 4v3h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-9a2 2 0 0 1 2-2h2V6a4 4 0 0 1 4-4zm0 2a2 2 0 0 0-2 2v3h4V6a2 2 0 0 0-2-2z" />,
    plus: <path d="M12 5v14M5 12h14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />,
    search: <path d="M11 3a8 8 0 1 0 0 16 8 8 0 1 0 0-16zm10 18l-4.35-4.35" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />,
    mic: <path d="M12 2a3 3 0 0 1 3 3v6a3 3 0 0 1-6 0V5a3 3 0 0 1 3-3zM19 11a7 7 0 0 1-14 0M12 19v3M8 22h8" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />,
    ...
    file: <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM14 2v6h6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" />,
  };
...
function App() {
  ...
  async function syncSource(source, limit = 1000) {
    ...
    setBusy(source);
    try {
      const url = source === "cves" ? `${routeMap[source]}?limit=${limit}` : routeMap[source];
      await api(url, token, { method: "POST" });
      await bootstrap(token, true);
    } finally {
      setBusy("");
    }
  ...
  if (!token) {
    return (
      <div className="login-screen">
        <Suspense fallback={<div className="loading-universe"><div className="gold-spinner" /></div>}>
          <UniverseBackground />
        </Suspense>
        <div className="login-card cinematic-card">
          <img src={LOGO_URL} alt="Hanicar Security" className="login-logo cinematic-logo" />
          <p className="eyebrow cinematic-eyebrow">Hanicar Security</p>
          <h1 className="cinematic-title">H-Brain</h1>
          <p className="lede cinematic-lede">Production-ready CTI, SOC operations, enrichment, and banking-grade incident scoring.</p>
          <form onSubmit={handleLogin} className="cinematic-form">
            ...
            <button type="submit" className="cinematic-btn">Launch H-Brain</button>
          </form>
  ...
  return (
    <div className="firebase-shell exact-shell">
      ...
      <aside className="sidebar firebase-sidebar exact-sidebar cinematic-sidebar">
        <div className="sidebar-scroll">
          <div className="brand-block brand-logo-block cinematic-brand">
            <img src={LOGO_URL} alt="Hanicar Security" className="brand-logo cinematic-brand-logo" />
            <div><p className="eyebrow cinematic-eyebrow">Hanicar Security</p><h2 className="cinematic-h2">H-Brain</h2></div>
          </div>
          <p className="nav-caption">Main Menu</p>
          <nav className="main-nav">{NAV_ITEMS.map((item) => <button key={item.id} className={`nav-item ${activeNav === item.id ? "active cinematic-active" : ""}`} onClick={() => { setActiveNav(item.id); setShowProfileMenu(false); }} type="button"><Icon name={item.icon} /><span>{item.label}</span></button>)}</nav>
          <div className="sidebar-lower-links"></div>
        </div>
      </aside>
      <main className={`firebase-main exact-main cinematic-main ${isDiscussion ? "discussion-mode" : ""}`}>
        {!isDiscussion ? <>
          <header className="topbar exact-topbar cinematic-topbar">
            ...
            <div className="topbar-actions compact-actions">
              <div className="notification-group" ref={notificationRef}>
                ...
                {showNotifications ? <div className="notification-popover scrollable-dropdown"><div className="dropdown-header"><div><strong>Notifications</strong><small>{notifications.total} unread or active</small></div><button className="ghost-button tiny-button" onClick={markAllNotificationsRead} type="button">Mark all read</button></div>{notifications.items.length ? notifications.items.map((item) => <button key={item.id} className={`notification-item ${item.is_read ? "read" : ""}`} onClick={() => markNotificationAndOpen(item)} type="button"><strong>{item.title}</strong><span>{item.body}</span><small>{item.severity.toUpperCase()} · {formatDate(item.created_at)}</small></button>) : <p className="empty-note">No notifications available.</p>}</div> : null}
              </div>
              <div className="profile-group" ref={profileRef}>
                <button className="avatar-chip dropdown-trigger" onClick={() => { setShowProfileMenu((current) => !current); setShowNotifications(false); }} type="button"><img src={LOGO_URL} alt="Hanicar avatar" className="cinematic-avatar" /><span>Admin</span><Icon name="chevron" /></button>
                {showProfileMenu ? <div className="profile-menu cinematic-profile-menu"><button type="button" onClick={() => { setActiveNav("settings"); setShowProfileMenu(false); }}>Settings</button><button type="button" onClick={logout}>Logout</button></div> : null}
              </div>
            </div>
          </header>
          {activeNav === "overview" ? <section className="stats-grid firebase-stats exact-stats">{stats.map((stat) => <article key={stat.label} className="stat-card firebase-card exact-stat-card cinematic-stat-card"><span>{stat.label}</span><strong>{stat.value}</strong><small>{stat.meta}</small></article>)}</section> : null}
        </> : null}
