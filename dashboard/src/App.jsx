...
function App() {
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel premium-chat"><div className="chat-interface"><div className="chat-header"><div><h3>H-Brain Expert</h3><p>Security Engineering & Operations</p></div><div className="chat-status"><span className="status-dot"></span>H-Brain Active</div></div><div className="chat-thread" ref={chatScrollRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}><div className="chat-content-container">{chatMessages.map((message, index) => {
