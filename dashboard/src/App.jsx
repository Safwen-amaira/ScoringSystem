...
function App() {
  ...
  useEffect(() => {
    if (!chatThreadRef.current || !autoScrollRef.current) return;
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel premium-chat"><div className="chat-interface"><div className="chat-header"><div><h3>H-Brain Specialist</h3><p>Security Engineering & SOC Operations</p></div><div className="chat-status"><span className="status-dot"></span>Active Reasoning</div></div><div className="chat-thread" ref={chatThreadRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}>{chatMessages.map((message, index) => {
    ...
    return <article key={`${message.role}-${index}`} className={`conversation-bubble ${message.role}`}><div className="bubble-meta"><span>{isAssistant ? "H-Brain" : "Operator"}</span></div><div className="bubble-content">{thought ? <details className="chat-thought" open><summary>Strategic Reasoning</summary><p>{thought}</p></details> : null}<p>{answer}</p></div></article>;
  })}{busy === "chat" ? <article className="conversation-bubble assistant loading"><span>H-Brain Specialist</span><p className="thinking-dots">Deep Analysis</p></article> : null}</div><form className="chat-input-area" onSubmit={handleChatSubmit}><div className="input-wrapper"><textarea value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleChatSubmit(event); } }} placeholder="Direct access to H-Brain: Ask about triage, containment, or banking SOC playbooks..." /></div><button type="submit" className="send-btn"><Icon name="chat" /><span>Send</span></button></form></div></section> : null}
