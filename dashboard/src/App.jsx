...
function App() {
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel"><PanelHeader eyebrow="AI Assistant" title="Cybersecurity engineering assistant" actions={<span className="mini-badge">H-Brain 0.2.0 (Reasoning)</span>} /><div className="chat-thread" ref={chatThreadRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}>{chatMessages.map((message, index) => {
    ...
    return <article key={`${message.role}-${index}`} className={`chat-bubble ${message.role}`}><span>{isAssistant ? "H-Brain" : "You"}</span>{thought ? <details className="chat-thought"><summary>Reasoning Process</summary><p>{thought}</p></details> : null}<p>{answer}</p></article>;
  })}{busy === "chat" ? <article className="chat-bubble assistant"><span>H-Brain</span><p className="thinking-dots">Thinking about your security posture</p></article> : null}</div><form className="chat-form" onSubmit={handleChatSubmit}><textarea value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleChatSubmit(event); } }} placeholder="Ask about triage, containment, threat hunting, Wazuh, MISP, Cortex, IRIS, MITRE, or banking incident response." /><button type="submit">Consult H-Brain</button></form></section> : null}
