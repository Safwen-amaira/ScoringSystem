...
function DataTable({ columns, rows, empty = "No records available.", onRowClick }) {
  return <div className="table-wrap"><table><thead><tr>{columns.map((column) => <th key={column.key}>{column.label}</th>)}</tr></thead><tbody>{rows.length ? rows.map((row, index) => <tr key={row.id || row.external_id || row.source_id || index} onClick={onRowClick ? () => onRowClick(row) : undefined} style={onRowClick ? { cursor: "pointer" } : undefined}>{columns.map((column) => <td key={column.key}>{column.render ? column.render(row) : row[column.key]}</td>)}</tr>) : <tr><td colSpan={columns.length} className="empty-cell">{empty}</td></tr>}</tbody></table></div>;
}
    ...
    return (
      <div style={{marginTop:"1rem", marginBottom:"1rem", padding:"1.25rem", background:"linear-gradient(135deg, #1a1f2e 0%, #12151a 100%)", borderRadius:"14px", border:"1px solid #2a3040"}}>
        <div style={{display:"flex", alignItems:"center", gap:"0.5rem", marginBottom:"0.75rem"}}>
          <h4 style={{margin:0, color:"#e0e6ed", fontSize:"0.95rem"}}>Summary</h4>
        </div>
