...
function App() {
  ...
  {activeNav === "discussion" ? <section className="panel firebase-card page-panel discussion-panel premium-chat hubspot-shell"><div className="chat-interface"><header className="hubspot-chat-header"><div className="header-left"><h3>H-Brain Analysis</h3><Icon name="chevron" /><span className="badge-pill private-badge"><Icon name="lock" />Private</span></div><div className="header-right"><button className="icon-btn"><Icon name="edit" /></button><button className="hubspot-share-btn">Share</button><button className="icon-btn"><Icon name="more" /></button><button className="icon-btn" onClick={() => setActiveNav("overview")}><Icon name="close" /></button></div></header><div className="chat-thread hubspot-thread" ref={chatScrollRef} onScroll={(event) => { const node = event.currentTarget; autoScrollRef.current = node.scrollHeight - node.scrollTop - node.clientHeight < 48; }}><div className="chat-content-container">{chatMessages.map((message, index) => {
